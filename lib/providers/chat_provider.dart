...
class ChatProvider extends BaseChatProvider {
  ...
  Future<List<Message>> getAttachments(String chatId, int type) async {
    DocumentReference chatDocRef = fireStoreDb
        .collection(Paths.chatsPath)
        .doc(chatId); //get reference to the chat documents
    CollectionReference messagesCollection = chatDocRef.collection(
        Paths.messagesPath); //get reference to all teh messages in the chat
    final querySnapshot = await messagesCollection
  ...
  Future<String> createChatIdForUsers(
    ...
    CollectionReference usersCollection =
        fireStoreDb.collection(Paths.usersPath);
    String selfUid =
        (await userUidMapCollection.doc(selfUsername).get()).dataAsMap['uid'];
    String contactUid = (await userUidMapCollection.doc(contactUsername).get())
        .dataAsMap['uid'];
    ...
    DocumentSnapshot contactDocRef =
        await usersCollection.doc(contactUid).get();

    print("Data:- ${selfDocRef.data} , ${contactDocRef.data}");

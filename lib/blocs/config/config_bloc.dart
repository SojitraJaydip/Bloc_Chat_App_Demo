...
class ConfigBloc extends Bloc<ConfigEvent, ConfigState> {
  ...
  Future<void> mapConfigValueChangedToState(
      ConfigValueChanged event, Emitter<ConfigState> emit) async {
    SharedObjects.prefs.setBool(event.key, event.value);
    emit(ConfigChangeState(event.key, event.value));
  }

  Future<void> mapRestartAppToState(
      RestartApp event, Emitter<ConfigState> emit) async {

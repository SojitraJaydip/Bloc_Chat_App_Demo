...
import 'package:chat_app_elision/config/paths.dart';
...
class AuthenticationBloc
  ...
  Future<void> mapPickedProfilePictureToState(
      PickedProfilePicture event, Emitter<AuthenticationState> emit) async {
    emit(ReceivedProfilePicture(event.file));
  }

  Future<void> mapAppLaunchedToState(
      AppLaunched event, Emitter<AuthenticationState> emit) async {
    try {
      emit(AuthInProgress()); //show the progress bar
      final isSignedIn =
          authenticationRepository.isLoggedIn(); // check if user is signed in
      if (isSignedIn) {
  ...
  Future<void> mapClickedGoogleLoginToState(
      ClickedGoogleLogin event, Emitter<AuthenticationState> emit) async {
    ...
    try {
      ...
      print(stacktrace);
      emit(
          UnAuthenticated()); // in case of error go back to first registration page
    }
  }
  ...
  Future<void> mapLoggedInToState(
    ...
    final user = await userDataRepository.saveDetailsFromGoogleAuth(
        event.user); // save the gAuth details to firestore database
  ...
  Future<void> mapSaveProfileToState(
    ...
    await userDataRepository.saveProfileDetails(user.uid, profilePictureUrl,
        event.age, event.username); // save profile details to firestore
    emit(ProfileUpdated()); //redirect to home page
  }

  Future<void> mapLoggedOutToState(
      ClickedLogout event, Emitter<AuthenticationState> emit) async {
